...
/**
 * Integration tests for ReviewSettings using IntelliJ Platform components.
 * Tests default values, setter/getter behavior, change notifications, and state management.
 */
class ReviewSettingsTests : LightPlatformTest() {
    ...
    @Nested
    inner class SettingsChangeNotification {
    ...
    inner class StateManagement {
        ...
        @Test
        fun `test State is BaseState subclass`() {
            // Given: A state instance
            val state = ReviewSettings.State()

            // Then: Should be a BaseState
            assertTrue(state is com.intellij.openapi.components.BaseState)
        }
